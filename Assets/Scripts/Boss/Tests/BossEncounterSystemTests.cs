...
namespace Burmalda.Boss.Tests
{
    public class BossEncounterSystemTests
    {
        ...
        private static Fixture CreateFixture(int requiredEnergy, int d20Roll = 5)
        {
            ...
            var system = new BossEncounterSystem(grid, trail, mana, coins, pool, tracker, reason => runState.ReportBossDefeat(reason), _ => requiredEnergy);

            return new Fixture
            {
                Grid = grid, Trail = trail, Mana = mana, Coins = coins,
                Pool = pool, Tracker = tracker, RunState = runState, System = system
            };
        }
