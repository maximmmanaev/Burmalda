...
| `IntegrationTests/` | **Тестовая инфраструктура, не система PRD.** `CoreLoopIntegrationTests` (issue #29) — e2e-сборка core loop одним прогоном (тоннель → ловушки → множитель → Алтарь/Ритуал → Босс → возврат/кэш-аут → Лагерь), напрямую конструирует те же Системы в том же порядке, что и реальные Controller'ы, без Play Mode (недоступен в этой сессии). Заодно поймала и починила реальный баг компиляции: `Persistence.ProgressSnapshot` ссылался на несуществовавший `ArtifactCollection.RecordedIds` | [Спринт 9](https://github.com/maximmmanaev/Burmalda/milestone/9) ([issue #29](https://github.com/maximmmanaev/Burmalda/issues/29)) |
