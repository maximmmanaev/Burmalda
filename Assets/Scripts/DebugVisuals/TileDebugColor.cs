...
namespace Burmalda.DebugVisuals
{
    ...
    public static class TileDebugColor
    {
        ...
        /// <summary>
        /// УСТАРЕЛО (issue #193, владелец, 2026-09-01): задача «рычаг и
        /// ворота невидимы» когда-то сделала рычаг механизмом-приглашением,
        /// видимым всегда — насыщенный зелёный, не опасность. Позже владелец
        /// пересмотрел: "две разновидности механизма с разной видимостью
        /// научат игрока неверному правилу" — рычаг теперь скрыт той же
        /// сигнатурой, что и ловушки (см. <c>Resolve()</c>,
        /// <see cref="TriggerSignatureColor"/>). Константа оставлена для
        /// истории/возможного будущего экрана "точный тип раскрыт Идолом
        /// Чутья" — в <c>Resolve()</c> больше не используется.
        /// </summary>
        ...
        public static Color Resolve(TileVisualState state)
        {
            ...
            // механизм — та же сигнатура, что триггеры ловушек выше, не
            // отдельная всегда-видимая категория. LeverColor оставлен
            // (константа) на случай будущего экрана "точный тип раскрыт
            // Идолом", но Resolve() её больше не выбирает.
            if (state.IsLever && state.IsDangerSignatureRevealed) return TriggerSignatureColor;
