...
namespace Burmalda.Movement
{
    ...
    ///
    /// <b>Issue #262 (2026-09-14, «LavaWaveTrapSystem не меняет тип плитки на
    /// Lava, только неверную текстуру») — ревизия: волна теперь ставит ТОТ ЖЕ
    /// <see cref="LethalTrapType.Lava"/>, что статичная Лава с генерации, не
    /// отдельный C#-идентификатор.</b> Раньше (issues #216/#254) сознательно
    /// был отдельный <c>LethalTrapType.LavaWave</c> — обоснование см. в
    /// историческом разделе «Сверка с уже существующим кодом» docs/wiki/traps.md:
    /// «не путать плита физически лава с генерации и сюда сейчас доходит
    /// волна». Разбор бага с устройства показал, что это разделение само
    /// стало причиной другого бага: <c>DebugVisuals.TileArtKindResolver</c>/
    /// <c>TileDebugColor</c> не заводили отдельную ветку под
    /// <c>LavaWave</c> — она попадала в общую категорию
    /// <see cref="DebugVisuals.TileArtKind.TimedTrapActive"/> вместе со
    /// Стрелой/Бомбой/Лезвиями, единственная текстура которой изображает
    /// наконечник Стрелы. Владелец решил не заводить Волне отдельную
    /// текстуру (как получила Бомба, issue #260) — она и есть Лава,
    /// концептуально, просто распространяется динамически, а не сразу вся
    /// целиком с генерации; слить идентификаторы проще и честнее, чем
    /// поддерживать вечно отстающую копию ветвления резолверов.
    ///
    /// <b>Проверено явно, важно для доверия к issues #249/#251:</b> ДО этой
    /// задачи волна уже ФАКТИЧЕСКИ меняла <see cref="Tile.LethalTrap"/> —
    /// <see cref="Tile.TransitionToLethalTrap"/> вызывался с (неверным)
    /// значением <c>LavaWave</c>, не был no-op. Баг был ЧИСТО про то, КАКОЕ
    /// значение enum ставится (и, как следствие, какую текстуру это
    /// значение резолвит) — не про то, менялся ли тип вообще. #249 (Алтарь
    /// не сгорает от волны) и #251 (статичная Лава непроходима для
    /// <see cref="Generation.SegmentReachabilityValidator"/>) оба работают
    /// на уровне, не зависящем от ТОГО, какое конкретно значение
    /// <see cref="LethalTrapType"/> ставится рантайм-волной — #249 проверяет
    /// <see cref="Tile.IsAltar"/> до вызова <c>TransitionToLethalTrap</c>
    /// вообще, #251 работает на этапе генерации с <c>SegmentTileType</c>, не
    /// видит рантайм-волну в принципе. Ни один из фиксов не переоткрыт этой
    /// задачей — регрессионные тесты обоих остаются зелёными без единой
    /// правки (проверено).
    ///
    /// <b>Побочный эффект слияния (issue #258, «Лава — плитка, на которую
    /// можно наступить, но игрок проигрывает»):</b> раз волна теперь ставит
    /// тот же <see cref="LethalTrapType.Lava"/>, плиты волны автоматически
    /// наследуют её поведение — физически проходимы
    /// (<see cref="GridTraceTrail.CanAdvanceTo"/>/<see cref="GridTraceTrail.TryAdvanceTo"/>
    /// больше не считают <c>Lava</c> стеной), но летальны через тот же d20
    /// (<see cref="RunLifecycle.RunState.ResolveHazard"/>). Реализация не
    /// дублируется — переиспользуется целиком, эта задача не трогает
    /// <c>GridTraceTrail</c> вовсе. Если issue #258 ещё не смержена в ветку,
    /// куда мержится эта задача, плиты волны временно останутся стеной (как
    /// и любая другая <c>Lava</c> до того фикса) — тип/текстура при этом уже
    /// корректны, поведение шага доедет автоматически при мерже #258, без
    /// дополнительных правок здесь.
    ///
    ...
    public sealed class LavaWaveTrapSystem : IDisposable
    {
        ...
        private void OnTileDue(GridCoordinate coordinate)
        {
            ...
            for (var column = 0; column < _grid.Width; column++)
            {
                ...
                // issue #262: тот же тип, что у статичной Лавы с генерации —
                // не отдельный LethalTrapType.LavaWave (удалён), см.
                // doc-комментарий класса.
                tile.TransitionToLethalTrap(LethalTrapType.Lava);
            }
