...
{
    /// <summary>
    /// Ловушка «Бомба» (docs/wiki/traps.md, issue #214) — НЕ
    /// <see cref="ExplosiveTrapArmingSystem"/>/<see cref="LethalTrapType.Explosion"/>
    /// (ближайший по духу прецедент, но не совпадает ни по одному из трёх
    /// параметров — активация мгновенная, не через 2 хода; одна плита, не
    /// площадь 3×3; результат постоянный, не возврат в обычное состояние —
    /// см. docs/wiki/traps.md «Сверка с уже существующим кодом»). Отдельный
    /// C#-идентификатор — <see cref="LethalTrapType.BombBlast"/>.
    ///
    ...
    /// вторую параллельную бомбу (тот же приём, что <see cref="TimedTrapSystem"/>/
    /// <see cref="ArrowWaveTrapSystem"/>). Несколько одновременных бомб
    /// (разные триггеры) поддерживаются независимо друг от друга.
    /// </summary>
