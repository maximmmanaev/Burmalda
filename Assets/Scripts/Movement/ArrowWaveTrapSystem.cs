...
{
    /// <summary>
    /// Ловушка «Стрела» (docs/wiki/traps.md, issue #213) — НЕ
    /// <see cref="Core.TimedTrapType.Arrow"/>/<see cref="TimedTrapSystem"/>
    /// (та же тема ловушки, другая механика: одна плита-цель на реальном
    /// времени там, волна по нескольким плитам ряда, тикаемая ХОДАМИ, здесь —
    /// см. docs/wiki/traps.md «Сверка с уже существующим кодом»). Отдельный
    /// C#-идентификатор — <see cref="LethalTrapType.ArrowWave"/>.
    ///
    ...
    /// параллельную волну (тот же приём, что <see cref="TimedTrapSystem"/>).
    /// Несколько одновременно активных волн (разные триггеры) поддерживаются
